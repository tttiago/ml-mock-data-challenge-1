...
class Slicer(object):
    ...
    def generate_data(self, key, index):
        ...
        dt = 1.0 / 2048  # This definition limits the scope of this object
        index_step_size = int(self.step_size / dt)
        sidx = (index.start - self.n_slices[key]["start"]) * index_step_size
        eidx = (
            (index.stop - self.n_slices[key]["start"]) * index_step_size + self.slice_length + 512
        )
        ...
        for detnum, rawdat in enumerate(rawdata):
            for i in range(index.stop - index.start):
                ...
                ts = ts.whiten(0.5, 0.25, low_frequency_cutoff=18.0)
...
def generate_dataset(samples, verbose=False):
    ...
    psds = [psd_fun(1281, 4.0 / 5.0, 18.0) for _ in range(len(detectors))]
    ...
    ### Create labels
    label_wave = np.array([1.0, 0.0])
    label_noise = np.array([0.0, 1.0])

    ### Generate data
    datasets = []
    num_waveforms, num_noises = samples
    logging.info(
        ("Generating dataset with %i injections and %i pure " "noise samples")
    ...
    for i in iterable:
        ...
        if is_waveform:
            ...
            waveform_kwargs = {"delta_t": 1.0 / 2048.0, "f_lower": 18.0}
            waveform_kwargs["approximant"] = "IMRPhenomD"
            masses = np_gen.uniform(10.0, 50.0, 2)
            waveform_kwargs["mass1"] = max(masses)
            waveform_kwargs["mass2"] = min(masses)
            angles = np_gen.uniform(0.0, 2 * np.pi, 3)
            ...
            time_interval = (
                time_interval,
                time_interval + 1.249,
            )  # 1.499 to not get a too long strain
                ...
                sum(
                    [
                        pycbc.filter.matchedfilter.sigmasq(
                            strain, psd=psd, low_frequency_cutoff=18.0
                        )
                        for strain, psd in zip(strains, psds)
                    ]
                )
            )
            target_snr = np_gen.uniform(5.0, 15.0)
            sample = (
                noise
                + np.stack([strain.numpy() for strain in strains], axis=0)
                * target_snr
                / network_snr
            )
        # If in the second part of the dataset, merely use pure noise as the full sample
            ...
            sample = noise
        # Whiten
        sample = [pycbc.types.TimeSeries(strain, delta_t=1.0 / 2048.0) for strain in sample]
        sample = [
            strain.whiten(0.5, 0.25, remove_corrupted=True, low_frequency_cutoff=18.0)
...
def get_clusters(triggers, cluster_threshold=0.35):
    ...
    logging.info(
        "Clustering has resulted in %i independent triggers. Centering triggers at their maxima."
        % len(clusters)
    )
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, samples, labels, store_device="cpu", train_device="cpu"):
...
def train(
    ...
    with open(os.path.join(output_training, "losses.txt"), "w") as outfile:
        ...
        for epoch in iterable1:
            ...
            iterable2 = TrainDL
            iterable2 = (
                tqdm(iterable2, desc="Iterating over training dataset", leave=False)
                if verbose
                else iterable2
            )
            ...
            with torch.no_grad():
                ...
                iterable2 = ValidDL
                iterable2 = (
                    tqdm(iterable2, desc="Computing validation loss", leave=False)
                    if verbose
                    else iterable2
                )
            ...
            output_string = "%04i    %f    %f" % (
                epoch,
                training_running_loss / training_batches,
                validation_loss,
            )
        ...
        logging.debug(
            (
                "Training complete with best validation loss "
...
def get_triggers(
    Network, inputfile, step_size=0.1, trigger_threshold=0.2, device="cpu", verbose=False
):
        ...
        for slice_batch, slice_times in iterable:
            with torch.no_grad():
                ...
                for slice_time, trigger_bool, output_value in zip(
                    slice_times, trigger_bools, output_values
                ):
                    if trigger_bool.clone().cpu().item():
                        triggers.append(
                            [slice_time.clone().cpu().item(), output_value.clone().cpu().item()]
                        )
        logging.info(
            "A total of %i slices have exceeded the threshold of %f."
...
def main():
    parser = ArgumentParser(
        description="Basic example CNN training and corresponding search script supplied for the MLGWSC-1."
    )
    ...
    testing_group.add_argument(
        "outputfile",
        type=str,
        help="The path where to store the triggers. The file must not exist.",
    )
    testing_group.add_argument(
        "-w",
        "--weights",
        type=str,
        help="The path to the file containing the network weights. If the --train option is present, the trained weights are used instead. Default: %s."
        % default_weights_fname,
    )
    testing_group.add_argument(
        "-t",
        "--trigger-threshold",
        type=float,
        default=0.2,
        help="The threshold to mark triggers. Default: 0.2",
    )
    testing_group.add_argument(
        "--step-size",
        type=float,
        default=0.1,
        help="The sliding window step size between analyzed samples. Default: 0.1",
    )
    testing_group.add_argument(
        "--cluster-threshold",
        type=float,
        default=0.35,
        help="The farthest in time that two slices can be to form a cluster. Default: 0.35",
    )
    testing_group.add_argument(
        "--device",
        type=str,
        default="cpu",
        help="Device to be used for analysis. Use 'cuda' for the GPU. Also, 'cpu:0', 'cuda:1', etc. (zero-indexed). Default: cpu",
    )
    ...
    training_group.add_argument(
        ...
        "--output-training",
        type=str,
        help="Path to the directory where the outputs will be stored. The directory must exist.",
    )
    training_group.add_argument(
        "--training-samples",
        type=int,
        nargs=2,
        default=[10000, 10000],
        help="Numbers of training samples as 'injections' 'pure noise samples'. Default: 10000 10000",
    )
    training_group.add_argument(
        "--validation-samples",
        type=int,
        nargs=2,
        default=[2000, 2000],
        help="Numbers of validation samples as 'injections' 'pure noise samples'. Default: 2000 2000",
    )
    training_group.add_argument(
        "--learning-rate",
        type=float,
    ...
    training_group.add_argument(
        ...
        type=str,
        default="cpu",
        help="Device to train the network. Use 'cuda' for the GPU. Also, 'cpu:0', 'cuda:1', etc. (zero-indexed). Default: cpu",
    )
    training_group.add_argument(
        "--store-device",
        type=str,
        default="cpu",
        help="Device to store the datasets. Use 'cuda' for the GPU. Also, 'cpu:0', 'cuda:1', etc. (zero-indexed). Default: cpu",
    )
